...
def regularized_linear_regression(X, y, lambd):
  ...
  #				 YOUR CODE HERE					                    #
  D = X.shape[1]
  w = np.linalg.pinv((X.T).dot(X) + lambd * np.identity(D)).dot(X.T).dot(y.T)
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  bestlambda = lambds[0]
  max_error = 1e10
  for single_lambda in lambds:
    w = regularized_linear_regression(Xtrain, ytrain, single_lambda)
    valid_error = test_error(w, Xval, yval)
    # print("single_lambda = ",single_lambda,", valid_error = ",valid_error)
    if valid_error < max_error:
      max_error = valid_error
      bestlambda = single_lambda
