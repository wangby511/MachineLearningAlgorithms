...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(np.ones((1,grad.shape[0])),grad)
        backward_output = np.dot(grad, np.array(self.params['W']).T)
...
class relu:
    ...
    def forward(self, X):
        ...
        """
        forward_output = np.maximum(X, np.zeros((X.shape)))
        ################################################################################
    ...
    def backward(self, X, grad):
        ...
        backward_output = grad
        backward_output[X < 0] = 0.0
        ##########################################################################################################################
